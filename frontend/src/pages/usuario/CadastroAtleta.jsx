// C:\Users\lenon\Development\evpc-torneio\frontend\src\pages\login\Cadastroatleta.jsx
...
function Cadastroatleta() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate('/login-atleta'), 2000);
    } catch (error) {
    ...
    <div className="min-h-screen bg-[url('/assets/bg-praia.png')] bg-cover bg-center bg-no-repeat flex items-center justify-center px-4">
      <div className='bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-8 w-full max-w-md'>
        ...
        <p className='text-sm text-blue-800 mt-4 text-center'>
          Já tem uma conta?
          <Link to='/login-atleta' className='text-yellow-600 font-semibold hover:underline ml-1'>
            Faça login
          </Link>
        </p>
      </div>
